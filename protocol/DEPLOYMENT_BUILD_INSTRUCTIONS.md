...
The main orchestrator and every nested/deployment coordinator retain that role across messages, synchronous answers, events, continuation/reconnect/compaction and approval. Apply [section 2 role/action admission](FIRST_SESSION_AND_ORCHESTRATION.md#persistent-role-and-action-admission) before tools/skills or substantive investigation. A child receives explicit parent tasks/scope/resources/budget and inherited or tighter settings/consent/authority, not execution permission or a competing ledger. Even one source investigation, domain-docs lookup, quick config edit, build/test/install/plan/apply/deploy/merge/debug call or execution skill belongs to a real executor. The engineering imperatives below direct execution workers; coordinators route and consume concise reports. A role rename or shell backgrounding is not an ownership handoff.

Coordinators may answer from held evidence/brief known concepts or truly compact authoritative coordination state, update coordination records, check dependencies/authority, dispatch and consume short candidate-specific evidence. Existing owners receive genuine versioned steering through demonstrated controls, including while busy; future independent tasks stay in durable backlog until admission. Scope a multi-lane capability to a sub-orchestrator that delegates to executors, not a synchronous whole-job wait. On drift, stop new execution, preserve handles/effects/write ownership and arrange supported safe observation/transfer without abandonment, blind cancellation or duplication.
...
Apply its [new-assignment admission](FIRST_SESSION_AND_ORCHESTRATION.md#admit-new-assignments-not-chat-backlog) to deployment lanes: default one active assignment and at most one unacknowledged assignment dispatch per worker, zero pending task backlog in chat. Reserve task/target/resources/budget and persist assignment/version/dispatch identity before send. Busy, dispatching, recovering, uncertain or resource-unavailable workers receive no additional task; answers and changed constraints/cancellations use demonstrated control/priority routes or exact assisted steps. A chat IDLE label does not release a surviving apply or migration's resources. An optional READY reply proves only that interaction; supported session creation may carry the complete approved packet without a second readiness roundtrip. Record actual receiver acceptance before effects and actual execution separately; a missing ACK cannot block pickup of an otherwise evidenced current result.
...
When suitable autonomous session creation or persistent execution is unavailable, prepare a complete worker launch packet for a separately opened execution session. Include the approved `run_policy_ref`, exact `effective_config`, consent and `budget_reservation`; verify manually applied model/reasoning settings before effects. Give the operator the exact supported steps to open that session and return its acknowledgement. Record delivery, receiver acceptance and actual execution separately in the canonical assignment record. Missing ACK leaves receipt uncertain, not proof that the job is inactive: reconcile late start/results without waiting for a lost event. Continue independent preparation through available workers. The coordinator must never run the job itself as a fallback, and it must not claim the manually opened session exists or is active without evidence.

## 2. Discover the deployment contract

Execution workers create a concise deployment contract from inspected project facts and user requirements. Coordinators use only compact coordination/capability state or needed approved bootstrap/configuration references for the interview; no source or domain investigation. Discovery workers and external LLM research must first pass the section 1 run gate:
...
2. **Define compatibility and inputs:** record supported Copilot surfaces, verified configuration formats, required tools, instruction discovery scope, relevant skill prerequisites, and project-specific values. Obtain exact approved supported model IDs, role defaults/overrides/fallbacks, and per-model reasoning minimum/default/maximum (or accepted fixed `N/A`) from run policy; do not hard-code model versions, rely on silent host defaults, or invent tool identifiers.
   An approved setup executor adapts the active root/child coordinator and executor entrypoints, core, role packets and native tool profiles only where supported. Apply the section 2 gate; expose only coordination operations to coordinators and verify actual state-write scoping where possible. Generic execution tools mean instructional limits. Record higher-priority host conflicts and supported assisted routes; do not invent configuration/hooks or promise cross-host enforcement or prevention of long reasoning/stalls.
...
5. **Activate and verify:** after explicit run approval and reservations, use assigned execution sessions in a disposable project or isolated checkout to test installation, then confirm that a fresh intended session discovers the right instructions, roles, run/approval/ledger pointers, effective settings, and relevant skills. Exercise dispatch, delivery acknowledgement, a bounded assignment, a recoverable long job where supported, and the recovery packet. Verify the actual execution and status interfaces for every supported Copilot surface; distinguish native delegation from manual session launch. Test the documented fallback when automatic delegation is absent.
   Verify loaded active role/core/profile in a fresh target root, child coordinator and spawned executor non-destructively. Use the main protocol's routing cases; label unsupported or unexercised restrictions rather than claiming downloaded references or simulated handoff prove activation.
...
Store actionable release intent in the canonical durable task/dispatch records. The Copilot chat queue is a delivery channel, never the intended task scheduler; internal host queueing remains a disclosed control limitation. Persist each intent's identity, scope, task reference, candidate, target, preconditions, assignment and reservations before send, then append actual acknowledgement observations. Use the project's duplicate-prevention mechanism where available. Do not claim exactly-once dispatch merely because an intent has an ID.

Apply the [idle-delivery safety contract](FIRST_SESSION_AND_ORCHESTRATION.md#idle-delivery-safety-contract) before relying on later release results. Publish candidate/results and actual target-effect evidence outside the recipient queue, then notify; "sent immediately" proves neither receiver receipt nor pickup. Demonstrate active-to-idle delivery and required closed-session resumption within approved safe scope. Arm the independent observer or obtain explicit operator-carried handoff acceptance with receipt/pickup windows, cursor, alternate alert/control route, recovery authority and inherited reservation. Register observation and recheck pending state before coordinator yield; do not create a deployment-specific scheduler or poll the main chat.

On missed receipt after a normal response ends, preserve accessible UI intent before disruption and use verified independent open/resume/reattach controls, not more queued recovery prompts. If recovery needs a new coordinator, establish/fence ownership under the main protocol before effects. Retrieve and reconcile the existing release result and actual target state before considering any replay. A late success notification must not republish, rerun a migration, duplicate reservations or broaden release authority. Observer loss or exhausted attempts requires an alternate operator-visible alert and scoped assisted hold, not unlimited retries or silent abandonment of a stateful operation.
...
Stop repeatedly sending the same intent into the affected channel. Under the canonical [uncertain-dispatch rule](FIRST_SESSION_AND_ORCHESTRATION.md#uncertain-dispatch-is-not-failed-execution), missing ACK, errors or unexplained idle quarantine new assignments to that lane, not the task's outcome. Retain ownership, budget and reservations; do not automatically revoke, retry once, or cancel a job. Preserve outstanding intent while answers and recovery/stop controls use demonstrated independent routes. Before treating other release work as independent or dispatching new effects, reconcile captured cancellations and changed constraints across all connections: pause new affected effects on the actual targets/resources, preserve live-job records, and reconcile running operations before attempting cancellation. Continue independent targets, runnable preparation, and result integration through functioning workers and supported channels. If workers finish but their messages are not consumed, retrieve their existing evidence before launching replacement work. If all useful work is waiting, record each dependency and the actual wake-up/observation owner; do not report normal progress or invent an automatic wake-up mechanism.
...
After independently reconciling actual session identity and current canonical ownership, reopen preserved state and compare each intent against receiver acceptance, launch evidence, backend jobs and target effects. An optional readiness handshake does not establish ownership or reliable later delivery. Apply only valid, unapplied intent under the main protocol's deduplication, ordering and new-assignment admission rules; keep undispatched tasks in the ledger, not a bulk chat replay. Newer cancellation/constraints suppress incompatible old effects. Never blindly replay create, migration, publish or traffic-change actions; reconnect is not cancellation or retry of an apply. Reattach observation to surviving operations and consume completed output without repeating mutations. Reassignment requires proven nonexecution with no possible obsolete late start, or safe stop/surrender/fencing at the actual conflicting write boundary and reconciliation of effects/charges. Provider idempotency covers only its verified scope, not all worker writes; ledger revocation is not fencing. Keep unresolved effects blocked locally and allow independent work. Late obsolete ACK/start cannot restore ownership; preserve old outputs for explicit revalidation, not automatic acceptance.
...
| Idle Copilot with queued intent | Sender success without receiver receipt triggers the armed independent observer within its approved window. Preserve intent/results and use the state-appropriate independent resume/manual route, without repeating release effects or blocking unaffected lanes. |
| Idle transition or observer loss | Registration plus pending-state recheck covers a result arriving around yield. Required closure survival is demonstrated separately; observer loss/exhausted recovery produces the supported alternate alert or explicit assisted hold, not self-wake claims. |
...
13. Who observes long jobs and missed receiver receipt/result pickup, what finite windows and recovery allowance apply, and what independently verified mechanism or accepted operator action resumes work when a coordinator is idle or gone? Where are original intents/results preserved before notification, and how is observer failure surfaced outside the broken queue?
