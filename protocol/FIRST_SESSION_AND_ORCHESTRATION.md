...
- **Continue authorized work in role.** Preserve authority with its scope and source. Coordinators route and accept; workers produce concrete reviewable results. Ask only for a genuinely missing decision or action beyond that authority.
- **Approve the run before dispatch.** Complete the mandatory configuration interview and record explicit approval before any orchestrated worker, including discovery/research, or direct external LLM request starts. Section 2 permits bounded coordination/interview preparation in the current chat, not domain investigation; this does not retroactively block that chat.
...
| Sub-orchestrator or deployment coordinator | Parent-assigned subtree, its task dependencies and delegated executor handoffs | Same coordinator boundary; no competing project-wide authority or independent ledger. |
...
### Persistent role and action admission

The responsible root orchestrator stays a **coordinator** across every user message, synchronous answer, steering, tool/result/completion event, continuation, reconnect, context compaction and approved run. "Do/fix/change it", a domain question or an answered choice changes **intent, not role**. A role change requires an explicit supported ownership/coordination handoff under valid authority; self-renaming while retaining coordination is not a handoff. An approved execution worker stays an executor and executes its assignment, without importing the coordinator's interview or preparation stopping rules.

**Before initiating any tool or skill, or substantive research/planning, admit the next action by kind and ownership, then route it under section 7.** Duration alone never grants permission. The coordinator may reason about priorities/contracts, answer briefly from already-held evidence or known conceptual knowledge with honest uncertainty, read truly bounded compact authoritative coordination/state/capability records, update coordination records and assess short candidate-specific worker reports. A needed approved public bootstrap/configuration reference may support coordination or the interview. These are not permissions to read implementation files or research a domain. Use existing context first; one compact targeted read must advance a coordination decision. Multiple small allowed reads must not accumulate into a domain investigation; insufficient evidence becomes a worker assignment or an exact evidence/access question.

Source scans or implementation-file reads to investigate, domain documentation lookups, benchmarks, security review, design evaluation, code/configuration/infrastructure/instruction edits as implementation, builds, tests, installs, deployments, merges, debugging and background execution belong to workers. Even one research call or a quick source fix is execution. Detailed evidence inspection belongs to a review worker; the coordinator consumes its concise candidate-specific report. Domain execution skills are selected in routed worker packets, not invoked by a coordinator because the user mentioned that domain. This applies to bootstrap, setup adaptation, recovery and testing orchestration itself; compact coordination-state writes are not implementation writes.

### Existing owners and bounded sub-orchestrators

Use the compact task/owner index, not the codebase, to find the existing responsible worker or sub-orchestrator first. Record the new requirement/steering identity, version, source and authority; reconcile changes to that assignment and deliver genuine answers/corrections/controls through a demonstrated path. A busy owner may receive current-task control under section 9; an unrelated future task belongs in durable backlog until admission, not disguised steering or queued task chat. Unknown delivery quarantines new assignments, not reserved ownership or existing effects. Reconcile before retry, replacement or fencing.

If no suitable owner exists, create a versioned bounded execution assignment only when readiness, isolation, resources, authority and remaining approved budget permit. For a capability requiring multiple lanes, a sub-orchestrator gets an explicit parent scope, task set, resource boundaries and budget allocation, then delegates execution to real workers. It does not acquire project-wide coordination, independent budget totals or a competing ledger. Child models/reasoning, consent, authority, caps, reservations and retries inherit or tighten the parent. Shared writes have an actual scoped owner. No mandatory fan-out or default worker count is implied. Escalate true decisions to the root/user and report concise evidence and handles through proven return controls; do not block the root on the whole subtree. The root retains the user's goal, status and acceptance and can answer while work runs.

### Activation and drift containment

An approved setup worker adapts the **active** host-supported instruction entrypoint, runtime core, role packets and native role/tool profiles where supported; downloading references alone does not activate this contract. Where verified native controls exist, allow coordinators only orchestration and bounded coordination-state operations; execution shells, general code writes, domain-docs research and build/deploy tools belong to workers. Verify actual write scoping for state tools. If a generic tool still permits arbitrary execution, mark that part **instructional, not enforced**. Do not invent tool identifiers, config filenames/formats, hooks or cross-host compatibility. Verify loaded roles/profiles in a fresh target root, a child coordinator and a spawned executor through the section 17 non-destructive checks. Record higher-priority host defaults that force conflicting behavior and the supported configuration or assisted route; Markdown cannot override them. Tool filtering cannot guarantee short model reasoning or prevent a host-call stall.

Choose a control-call budget from measured runtime behavior and record it in project policy; permitted synchronous dispatch/status reads must fit it. If a coordination read exposes an investigation, stop expanding it and route it. On detected coordinator execution drift, stop starting further execution, preserve exact operation handles, observed and uncertain effects, write ownership and reservations, and arrange accountable observation/recovery. Transfer observation/execution to an authorized worker only where the actual transfer and safety reconciliation are supported. Do not blindly cancel stateful work, abandon a running operation, duplicate it, restart yourself or pretend an external supervisor exists. Continue eligible coordination through proven controls; report the exact transfer/control gap when blocked.
...
Apply the section 2 role/action gate before each step: "approved action" or "advance" for a coordinator means permitted coordination or dispatch, never execution. Use supplied evidence first. For a missing coordination/interview fact, allow one known-short targeted read of a named compact authoritative record, needed approved public bootstrap/configuration reference, or local capability metadata result. No implementation reads or domain-docs research qualify. Use its result to advance the question/action, not to force a terminal acknowledgement. If insufficient, name the precise gap; ask for its evidence/access or advance an independently answerable interview field. Do not chain repository scans, model catalogs, pricing research, instruction generation or capability drills under "safe local preparation." Unknown or potentially long calls are not bounded merely because there is only one of them. Bound preparation, not the number of answers a host may deliver. Do not repeat an unchanged lookup/question or poll for input.
...
5. **Adapt the target, not the reference cache.** Merge the section 2 persistent role/action gate into the actual host-supported entrypoint, compact runtime core, role packets and supported native role/tool profiles for root, child coordinators and executors. Supersede conflicting active continuation/prompt copies; do not append a competing rule. Verify profile support and scoped state writes or label the boundary instructional. Reuse POLICY/CAPABILITIES/PROJECT_STATE and the authoritative task/ledger arrangements. Separate immutable references from localized project facts and active rules; do not copy sample state as live state, create competing trackers, overwrite customizations, reset authority/budgets or orphan existing workers. An unapproved instruction conflict is a scoped hold.
6. **Verify adoption and continue the actual outcome.** Return source identity, localized paths, changed-rule summary, section 17 root/child/executor discovery and role evidence, remaining limits and next ready assignment. Exercise the approved non-destructive handoff path; copied files alone do not activate anything. Once the gates pass, the coordinator routes the user's implementation/build task to its existing or eligible new owner; the executor performs it. Downloading references or writing setup documents is not the requested software outcome.
...
| Role and tool admission | Fresh target root and child coordinators load the sticky role and section 2 gate; spawned executor loads an execution packet. Identify actually applied native tool profiles and state-write scope where supported; otherwise record instructional limits and higher-priority conflicts. Non-destructive routing checks do not prove hard enforcement or absence of long reasoning/stalls. |
...
Generate project-specific launch packets for the root coordinator, bounded sub-orchestrator, execution worker, review worker, integration worker, recovery worker, and deployment worker. Carry the persistent session role separately from model selection, the parent/scope/authority and the section 2 action gate; executor packets authorize execution, not coordinator interviews. Reuse supported custom-agent/tool-profile formats only where verified. Distribute the entrypoint and core to the branch/configuration future sessions actually use, and test discovery there.

## 6. Generate this compact runtime core

Adapt the following core with real project paths and verified mechanisms. Resolve its section 2 link to the localized detailed role contract, not a dangling fragment in the generated core. Keep it compact enough to load with current state and assignment; move explanations and completed history into linked records. This reference is not an always-loaded instruction.

> **Identity and intent.** Load the applicable entrypoint, core, compact state and assignment. Identify session role, parent scope, coordinator epoch, task/assignment/requirement versions, workspace, authority and remaining budget. Role persists across all messages, synchronous answers, events, continuation/reconnect/compaction and approval. User intent does not turn a coordinator into an executor; role change needs explicit supported ownership handoff. An approved executor executes its task, not the coordinator interview.
>
...
>
> **Role/action gate before tools or skills.** Apply the [section 2 routing contract](#persistent-role-and-action-admission) on every event before substantive investigation or a tool/skill. Root and child coordinators answer from held evidence/brief known concepts or one truly compact coordination-state read; otherwise record intent and route to the existing owner first, an eligible versioned executor, or a bounded parent-scoped sub-orchestrator. Ask only a real decision or exact control gap. No source investigation, domain-docs lookup, quick implementation edit, execution skill, build/test/deploy/merge/debug or background job in a coordinator. No research chains disguised as tiny reads. Workers own execution and detailed review. Use verified nonblocking controls or an approved independent-worker packet; missing dispatch is not executor permission. Native tool profiles are enforced only where verified; generic tools remain instructional. Preserve handles/effects/owners on drift and arrange safe accountable observation/transfer, never abandonment or blind cancellation.
>
> **Advance or wait in role.** Reuse supplied/prestaged answers; keep at most one unanswered question. Reconcile synchronous answers/events then take the next admitted coordination read, question, policy review/final approval or dispatch; approval never grants coordinator execution. "Continue pending" uses compact authoritative task/owner state, not a rewritten backlog. Preapproval reads are only needed compact coordination/capability metadata or approved bootstrap/configuration references; missing model evidence means an exact question/gap, not an unapproved discovery worker. A status-only exit is invalid while a permitted step remains. Follow actual host lifecycle: answered is not waiting; suspended is not completed. Yield for real waits with gap, owner and supported event/manual action after eligible steps. No polling, full-job waits or invented self-wake. Executors execute complete valid assignments.
>
...
On every user message and delivered event, retain the session role and apply [section 2 action admission](#persistent-role-and-action-admission) **before tools, skills or substantive investigation**. Determine whether it asks a question/status, refines current work, adds independent scope or explicitly replaces/cancels an objective. A recent message is not permission to discard earlier requirements or change role. If interpretation would materially change incompatible work, preserve what is known, continue independent coordination and ask the narrow unresolved question.

| Admitted route | Coordinator action |
| --- | --- |
| Answer from existing evidence | Give a short answer/status or conceptual explanation with uncertainty; use at most a truly bounded compact coordination-state read if needed. Source-dependent questions route to a worker, not local investigation. |
| Existing responsible owner | Look up compact task/owner state; record stable requirement/steering ID, version and authority. Deliver a current-task answer/correction through demonstrated controls, including to a busy owner. Reconcile receipt/application; do not disguise independent tasks as steering. |
| No suitable owner | Put independent scope in durable backlog; admit a versioned bounded executor assignment only after approval, ownership/isolation, resources and remaining budget pass. Do not duplicate uncertain existing work. |
| Multi-lane capability | Assign a bounded sub-orchestrator with explicit parent tasks/scope/resources/allocation and inherited or tighter policy. It coordinates real executors, not a competing project-wide ledger; return concise reports without root full-job waits. |
| Real choice or unavailable route | Ask only the missing material decision or name the exact supported control/approved operator-carried independent-worker handoff. Missing dispatch or approval never permits coordinator execution. |
...
1. **Reconcile and route:** use held evidence or compact authoritative state, new steering, result identities, active operations, ownership changes, approved run version, usage/reservations and uncertainty. Apply the route table and existing-owner-first rule before choosing tools or creating work.
...
session_role: <execution worker or coordinator; persists until authorized supported handoff>
role_contract_ref: <localized section 2 admission rules and verified tool profile or instructional limit>
parent_scope: <responsible coordinator; bounded subtree/tasks/resources if a child coordinator>
...
skills: <role-admitted references; domain execution skills belong to executor assignments>
...
| --- | --- |
| Native role/tool profile | Apply section 2 role admission using actual supported controls; scope coordination-state writes where possible. Generic execution-capable tools remain instructional exposure. No claim to limit model reasoning or host-call duration. |
...
| Sticky role and routing | In an approved non-destructive exercise, a fresh target root and child coordinator identify loaded active role/core/profile and route known-status, clarification, source-dependent question and "fix it" cases under section 2. A spawned executor executes a harmless assigned task without repeating the interview. Do not claim an unobserved reconnect/compaction path passed. |
| Tool-profile boundary | Inspect actual exposed tools and native state-write scope without destructive effects. Record enforced coverage only where demonstrated; unsupported allowlists/generic shells mean instructional limits. A higher-priority execution default is an explicit conflict with a supported configuration/assisted route, not a passing Markdown override. |
| Drift containment | Using a supplied harmless operation record, retain exact handle, effects, ownership and charges and choose the supported accountable observation/transfer path or exact gap. Do not launch a real stateful job just to cancel it or claim a simulated transfer proves real adoption. |
...
After setup, coordinators route the next ready authorized task under section 2; executors perform their approved assignments. Keep the operating view concise: objective and steering revision, selected route, accepted evidence, active assignments/operations, blocked scopes, capacity constraints, and next coordination action. Archive completed detail and remove conflicting or redundant rules through reviewable changes. Leave the project recoverable by a fresh coordinator without dependence on this conversation.
